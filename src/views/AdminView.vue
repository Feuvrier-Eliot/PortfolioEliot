  ...
  <div>
      ...
      <div class=" px-5 grid grid-cols-1 lg:grid-cols-2 gap-20 relative ">
          ...
          <form class="grid grid-cols-[minmax(max-content,8rem)_minmax(10rem,35rem)] gap-1 " @submit.prevent="onCreate">
              <fieldset class="contents">
                  <legend class="my-1 border-b-2 col-span-full text-black font-poppins textresponsive effet-shadowblanc pb-7">Créer un compte</legend>
                  <label class="text-black font-poppins textresponsive p-1">Login :</label>
                  <input type="text" class="rounded-sm border-2 text-black" v-model="user.login">
                  <label class="text-black font-poppins textresponsive p-1">Email :</label>
                  <input type="email" class="rounded-sm border-2 text-black" v-model="user.email">
                  <label class="text-black font-poppins textresponsive p-1">Mot de passe : </label>
                  <input type="password" class="rounded-sm border-2 text-black" v-model="user.password">
                  <label class="text-black font-poppins textresponsive p-1">Confirmer le mot de passe : </label>
                  <input class="rounded-sm border-2 text-black" type="password" v-model="password2"/>
                  <button class="bg-red-500 rounded-sm mt-5 text-black font-poppins textresponsive effet-shadowblanc p-3 " type="submit" >Créer</button>     
              </fieldset>
          </form>
      </div>
